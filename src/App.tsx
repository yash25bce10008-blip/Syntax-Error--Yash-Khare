...
function youtubeSearchUrl(career: CareerRole, stage: RoadmapStage) {
  return `https://www.youtube.com/results?search_query=${encodeURIComponent(`${career.title} ${stage.title} tutorial course`)}`
}

function youtubeEmbedUrl(career: CareerRole, stage: RoadmapStage) {
  return `https://www.youtube.com/embed?listType=search&list=${encodeURIComponent(`${career.title} ${stage.title} tutorial course`)}`
}
...
function StageDrawer({ stage, career, user, setUser, onClose }: { stage: RoadmapStage; career: CareerRole; user: UserProfile; setUser: (user: UserProfile) => void; onClose: () => void }) {
  ...
  const youtubeResource = stage.resources.find((resource) => resource.type === 'YouTube')
